 ...
 */
export default async function sendEmail({ gmail = false, to, bcc, replyTo, subject, body, from, attachments = [] }) {
    ...
    const bccString = Array.isArray(bcc) ? bcc.join(", ") : bcc;

    // Build headers 
    const headers = [
        from ? `From: ${from}` : undefined,
        `To: ${to}`,
        bccString ? `Bcc: ${bccString}` : undefined,
